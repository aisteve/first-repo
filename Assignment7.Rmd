...
```{r tempPrst, echo=FALSE}

str(tempPrst)
class(tempPrst$ageBeforeOffice)
class(tempPrst$ageAfterOffice)

p1 <- ggplot(tempPrst, aes(ageBeforeOffice, ageAfterOffice)) + geom_point() + geom_smooth(method='lm') + ggtitle("Analysis on President Age Group") + labs(x = 'Age Before Office', y= 'Age After Office')

p2 <- ggplot(tempPrst, aes(ageBeforeOffice, ageAfterOffice, color = party)) + geom_point() + geom_smooth(method='lm', se = FALSE, na.rm=TRUE) + ggtitle("Analysis on President Age Group of different parties") + labs(x = 'Age Before Office', y= 'Age After Office') + facet_wrap( ~ party)

p3 <- qplot(ageAfterOffice, ageBeforeOffice, data = tempPrst, color = "green", main = 'Analysis on President Age Group')

prst <- tempPrst

p3 <- p3 + xlab('Age After Office') + ylab('Age Before Office')
